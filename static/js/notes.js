...
class Controller {
    ...
    async initialise() {
        ...
        this.initialiseCreateEvent();
        this.initialiseUpdateEvent();
        this.initialiseDeleteEvent();
        this.initialiseResetEvent();
    }

    async initialiseTable() {
        try {
            let urlPersonId = +document.getElementById("url_person_id").value;
            let urlNoteId = +document.getElementById("url_note_id").value;
            let person = await this.model.read(urlPersonId);
    ...
    initialiseTableEvents() {
        document.querySelector("table tbody").addEventListener("click", (evt) => {
            ...
            let noteId = target.getAttribute("data-note_id");
